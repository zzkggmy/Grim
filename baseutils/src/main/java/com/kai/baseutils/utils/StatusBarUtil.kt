...
object StatusBarUtil {
    ...
    @JvmOverloads
    fun setColor(view: View, activity: Activity, @ColorInt color: Int, statusBarAlpha: Int = DEFAULT_ALPHA,isBlack: Boolean) {
        view.setBackgroundColor(color)
        changeStatusBarTextColor(activity,isBlack)
     ...
     * @param color    状态栏颜色值
     */
    fun setColorNoTranslucent(view: View,activity: Activity, @ColorInt color: Int,isBlack: Boolean) {
        setColor(view,activity, color, 0,isBlack)
    }
    ...
    private fun setRootView(activity: Activity) {
        ...
        while (i < count) {
            ...
            if (childView is ViewGroup) {
                if (Build.VERSION.SDK_INT > 14) childView.setFitsSystemWindows(true)
    ...
    fun obtainHeight(context: Context): Int {
        ...
        try {
            ...
            if (context is Activity) {
                ...
                if (Build.VERSION.SDK_INT > 14) context.window.decorView.getWindowVisibleDisplayFrame(frame)
                result = frame.top
            }
        }
